...
const Filter: React.FC<FilterProps> = ({
  ...
  const handleFilter = (e: any) => {
    ...
    const filteredData = passedData.filter((each) => {
      const { username, phonenumber, datejoined, email, status, organization } =
        each;
      ...
      return (
        username
          .toString()
          .toLowerCase()
          .includes(filterObject.username.toString().toLowerCase()) &&
        organization
          .toString()
          .toLowerCase()
          .includes(filterObject.organization.toString().toLowerCase()) &&
        email
          .toString()
          .toLowerCase()
          .includes(filterObject.email.toString().toLowerCase()) &&
        status
          .toString()
          .toLowerCase()
          .includes(filterObject.status.toString().toLowerCase()) &&
        formatDate(datejoined)
          .toString()
          .toLowerCase()
          .includes(formatDate(filterObject.date).toString().toLowerCase()) &&
  ...
  return (
    <form
      ...
      style={
        showFilter && selectedFilter === filterName
          ? { display: "flex" }
          : { display: "none" }
      }
